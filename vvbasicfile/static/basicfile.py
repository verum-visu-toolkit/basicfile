import vvbasicfile.utils
import json
import gzip


class BasicFormat(vvbasicfile.utils.SchematicAttributesObject):
    def __init__(self, data=None, packeddata=None, format_attrs=None):
        ...
        vvbasicfile.utils.SchematicAttributesObject.__init__(self, attrs=format_attrs)
        ...
        if type(packeddata) is dict:
            ...
            vvbasicfile.utils.update(self.data, data)
            self.validate(attr='data')

    # Files

    # IMPROVEMENT: combine these pairs of functions into load_from_file and write_to_file with
    # compressed option
    ...
    # STREAM PIPELINE: json -> [->fms/->rnd] -> json
    ...
    def update_config(self, config_dict, **config_args):
        vvbasicfile.utils.update(self.config, config_dict)
        vvbasicfile.utils.update(self.config, config_args)
    ...
    def _pack(self):
        self._packeddata = {
            'config': vvbasicfile.utils.copy(self.config),
            'data': vvbasicfile.utils.copy(self.data),
        }

    def _unpack(self):
        self.config = vvbasicfile.utils.copy(self._packeddata['config'])
        self.data = vvbasicfile.utils.copy(self._packeddata['data'])
